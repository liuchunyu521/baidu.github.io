...
function(w, s) {
    ...
    s.UMService = {
        ...
        call: function(serviceType, jsonArgs, isSync) {
            if (arguments.length === 1) {
                var _options = serviceType
                serviceType = "SummerService.gotoNative"
                var _transtype = "serviceCall";

                if (_options.params) {
                    var _innerParams = _options.params
                }
                ...
                function _callback_(data) {
                    ...
                    if (typeof(_options.callback) === "string") {
                        var _funcname_ = _options.callback.substring(0, _options.callback.indexOf("("));
                        var _callbackFn_ = eval(_funcname_);
                        _callbackFn_(data.result)
                    } else {
                        _options.callback(data.result)
                    }
                }

                function _error_(err) {
                    if (typeof(_options.error) === "string") {
                        var _funcname_ = _options.error.substring(0, _options.error.indexOf("("));
                        var _callbackFn_ = eval(_funcname_);
                        _callbackFn_(err)
                    } else {
                        _options.error(err)
                ...
                if (_innerParams) {
                    jsonArgs.params.innerParams = _innerParams
                }
                if (_options.viewid) {
                    jsonArgs.params.controllerId = _options.viewid
                }
                isSync = false
            }
            try {
    ...
    s.callAction = s.UMService.call;
    s.callActionMa = s.UMService.callAction;
